...
SPIN_TEST(betting_history_marks_blinds_forced){auto s=sc3(0);BettingEngine b(s,make_game_topology(s));REQUIRE(b.history().size()==2);REQUIRE(b.history()[0].forced);REQUIRE(b.history()[1].forced);REQUIRE(b.history()[0].pot_before==0);REQUIRE(b.history()[0].pot_after==10);REQUIRE(b.history()[1].pot_before==10);REQUIRE(b.history()[1].pot_after==30);auto l=b.legal_actions(0);auto ev=b.apply(0,{ExactActionType::RaiseTo,l.min_raise_to});REQUIRE(!ev.forced);REQUIRE(ev.pot_before==30);REQUIRE(ev.pot_after==b.pot());}
