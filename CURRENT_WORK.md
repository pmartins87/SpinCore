...
Date: 2026-09-18
Status: **LT2 STAGE B PASS — 4.5M ROOTS — HU-JAMMER REGRESSION LOCALIZED — K4 MECHANICS PASS — FINAL FACING-ALL-IN TARGET OVERLAY NEXT — NO TRAINING YET**
...
## Final pre-training causal gate
...
The overlay reconstructs actual forensic Jammer FACING_ALL_IN first-divergence states and compares Stage A/B on:
- AveragePolicy;
- current Advantage-induced policy;
- stage-specific high-budget conditional target reference;
- K1 versus K4 target estimator quality;
- FOLD/CHECK_CALL/ALL_IN mass;
- target-policy TV and regret.

It uses only the already-seen forensic seeds.

Reserved holdout seeds `20261001..20261006` remain untouched.

## Decision after overlay

Only if the same forensic states show:
1. Stage B AveragePolicy farther from target than Stage A;
2. Stage B Advantage policy farther from target in the same direction;
3. the action-mass shift matches the Jammer-facing regression;
4. K4 improves estimator quality on those exact states;

may a bounded K4 training pilot be considered.
...
Pull current `main` and run `bash tools/run_lt2_jammer_facing_allin_target_overlay.sh`.

Wait for `LT2_JAMMER_FACING_ALLIN_TARGET_OVERLAY_PASS` or the first error.

Then send `SpinCore_LT2_jammer_facing_allin_target_overlay.json`.
