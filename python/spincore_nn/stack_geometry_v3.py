...
class PairwiseStackGeometryV3:
    """Actor-relative stack geometry without a false single multiway SPR.

    V1 current-state numerics are already normalized by the current big blind:
      numeric[0]      pot / BB
      numeric[3:6]    remaining stacks / BB for actor-relative seats 0..2
      numeric[9:12]   total commitments / BB
    categorical[4:7] stores status 0 active, 1 folded, 2 all-in.

    True-HU callers must pass the already canonicalized layout
    `[Hero, live opponent, absent]`. In 3H, rel1/rel2 retain real positional
    identity. Values are not sorted by stack size because doing so would erase
    position.
    """
...
def derive_pairwise_stack_geometry_v3(item: DecodedInput) -> PairwiseStackGeometryV3:
    if len(item.numeric) != 16 or len(item.categorical) != 8:
        raise ValueError("unexpected V1 state dimensions for stack geometry")
    ...
    for rel in (1, 2):
        ...
        # In canonical true HU, rel2 is the absent chair by contract.  A real
        # all-in player is never inferred absent merely from stack==0 because
        # they have positive total commitment.  Three-handed seats remain
        # present even after folding/all-in.
