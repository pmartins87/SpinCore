...
$ToolBase = Join-Path $Repo 'tools/r7_5_arch_reset_v1plus_phase2c2_range_reach_target_kernel_causal_pilot.py'
...
& $Python $Test
if ($LASTEXITCODE -ne 0) { throw 'Phase2C2 structural synthetic tests failed.' }
...
& $Python -c "import hashlib,sys; from pathlib import Path; root=Path(sys.argv[1]); expected={1342191342:'f853dedf68a7f94b203d1f1de8b650897b2ddd69338fde56918d685f987b7cd3',1801739323:'ef25629fa8e99a024bd3c7f27d6a6140734e5efe3e5d245b390d3028a0e2cca3'}; out=[]; [(lambda s,e: (lambda p,h: (_ for _ in ()).throw(AssertionError((s,h,e))) if h!=e else out.append((s,h)))(root/'IID64_MEAN_CANDIDATE'/f'seed_{s}'/'resume_checkpoint.pt',hashlib.sha256((root/'IID64_MEAN_CANDIDATE'/f'seed_{s}'/'resume_checkpoint.pt').read_bytes()).hexdigest()))(s,e) for s,e in expected.items()]; print('Phase2B13 final candidate behavior checkpoints PASS',out)" $B13Root
...
Write-Host '[V1+ Phase2C2] proving positive-support depth-2 continuation across all scenarios/seeds...'
...
Write-Host '[V1+ Phase2C2] running one real fair-control K64 root + K64 structural continuation kernel preflight...'
...
Write-Host '[V1+ Phase2C2] fair control = one uniformly selected stratified cell; candidate = mean of identical K64 set.'
...
& $Python (Join-Path $Repo 'tools/spincore_ryzen_frozen_runner.py') `
    ...
    --run-name 'r7_5_arch_reset_v1plus_phase2c2_range_reach_target_kernel_causal_pilot_controlfair' `
