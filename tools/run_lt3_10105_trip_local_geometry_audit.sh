...
"${PY}" -m py_compile tools/audit_lt3_10105_trip_local_geometry.py
STAMP="$(date +%Y%m%d_%H%M%S)"
RUN="${ROOT}/runs/lt3_10105_trip_local_geometry/${STAMP}"
mkdir -p "${RUN}"
REPORT="${RUN}/trip_local_geometry_10105.json"

"${PY}" tools/audit_lt3_10105_trip_local_geometry.py --checkpoint "${CHECKPOINT}" --report "${REPORT}"
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  if [[ -n "${DESKTOP_WIN}" ]]; then
    ...
    if [[ -n "${DESKTOP_WSL}" && -d "${DESKTOP_WSL}" ]]; then
      cp -f "${REPORT}" "${DESKTOP_WSL}/SpinCore_LT3_10105_trip_local_geometry.json"
      echo "desktop_report=${DESKTOP_WIN}\\SpinCore_LT3_10105_trip_local_geometry.json"
    fi
  fi
fi
echo "LT3_10105_TRIP_LOCAL_GEOMETRY_RUN_COMPLETE"
