...
"$PY" -m py_compile tools/audit_lt2_hu_mature_ens4_broad_ev.py
echo "PYTHON_PREFLIGHT_PASS"

STAMP="$(date +%Y%m%d_%H%M%S)"
DIR="$ROOT/runs/lt2_hu_mature_ens4_broad_ev/$STAMP"
mkdir -p "$DIR"
REPORT="$DIR/hu_mature_ens4_broad_ev.json"
...
echo "=== SpinCore LT2 mature HU size-4 ensemble broad EV ==="
echo "8 x fresh400 from reservoir8000; ENS4 left/right; full forensic HU population"
echo "READ ONLY sources; NO CFR ROOTS; HOLDOUT UNTOUCHED"

"$PY" tools/audit_lt2_hu_mature_ens4_broad_ev.py   --solver "$SOLVER"   --stage-7500 "$P7500"   --stage-7600 "$P7600"   --stage-8000 "$P8000"   --budget 400   --scenarios-per-seed 5000   --workers 31   --threads-fit 8   --report "$REPORT"

"$PY" - "$REPORT" <<'PY'
import json,sys
from pathlib import Path
d=json.loads(Path(sys.argv[1]).read_text())
assert d["schema"]=="SPINCORE_LT2_HU_MATURE_ENS4_BROAD_EV_V1"
m=d["method"]
...
assert len(d["candidate_meta"])==8
assert set(d["summary"])=={"UNIFORM_LEGAL","PASSIVE_CALLER","JAMMER"}
for b,x in d["summary"].items():
    assert x["clusters"]>0
    assert set(x["ensembles"])=={"ENS4_LEFT","ENS4_RIGHT"}
print("LT2_HU_MATURE_ENS4_BROAD_EV_POSTVALIDATION_PASS")
PY
...
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_hu_mature_ens4_broad_ev.json"
fi

echo "LT2_HU_MATURE_ENS4_BROAD_EV_PASS"
echo "STOP HERE. Send SpinCore_LT2_hu_mature_ens4_broad_ev.json to ChatGPT."
