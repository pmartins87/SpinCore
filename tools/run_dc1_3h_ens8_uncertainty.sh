...
HU_ENSEMBLE="${SOURCE_RUN}/hu_ensemble_state.pt"
...
RUN="${ROOT}/runs/dc1_3h_ens8_uncertainty/${STAMP}"
mkdir -p "${RUN}"
BUNDLE="${RUN}/spincore_hybrid_10105.pt"
PROBE="${RUN}/three_handed_ens8_diagnostic_probe.pt"
REPORT="${RUN}/dc1_3h_ens8_uncertainty.json"
...
"${PY}" tools/build_3h_ens8_diagnostic_probe_10105.py \
  --solver build/libspincore_solver_c.so \
  --checkpoint "${CHECKPOINT}" \
  --out "${PROBE}" \
  --threads 8

"${PY}" tools/evaluate_dc1_3h_ens8_uncertainty.py \
  --solver build/libspincore_solver_c.so \
  --spin-bundle "${BUNDLE}" \
  --probe "${PROBE}" \
  --scenarios 200 \
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  if [[ -n "${DESKTOP_WIN}" ]]; then
    ...
    if [[ -n "${DESKTOP_WSL}" && -d "${DESKTOP_WSL}" ]]; then
      cp -f "${REPORT}" "${DESKTOP_WSL}/SpinCore_DC1_3H_ens8_uncertainty.json"
      echo "desktop_report=${DESKTOP_WIN}\\SpinCore_DC1_3H_ens8_uncertainty.json"
    fi
  fi
fi

echo "DC1_3H_ENS8_UNCERTAINTY_RUN_COMPLETE"
