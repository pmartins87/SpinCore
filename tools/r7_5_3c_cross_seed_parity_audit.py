...
def _assert_close(left: float, right: float, name: str, tol: float = 1e-12) -> None:
...
def main() -> int:
    ...
    for path in sorted(args.cells_root.rglob("cell.json.gz")):
        ...
        if payload.get("schema") != HELDOUT_SCHEMA:
            continue
        if payload.get("evaluator_execution_sha") != args.evaluator_sha:
            raise RuntimeError(f"evaluator SHA mismatch: {path}")
        if payload.get("training_execution_sha") != args.training_sha:
            raise RuntimeError(f"training SHA mismatch: {path}")
        if payload.get("heldout_execution_sha") != args.heldout_sha:
            raise RuntimeError(f"heldout SHA mismatch: {path}")
        key = (
            str(payload["representation"]),
            str(payload["domain"]),
            int(payload["training_seed"]),
            int(payload["evaluation_seed"]),
        )
        if key in cells:
    ...
    for path in sorted(args.policies_root.rglob("*.pt")):
        ...
        rep = str(payload.get("representation"))
        ...
        policy = load_finalized_v3_policy_light(
            ...
            expected_training_execution_sha=args.training_sha,
    ...
    results = []
    global_max_recompute_abs = 0.0
    for rep in (H2_FINAL, H3_FINAL):
        for domain in DOMAINS:
            for evaluation_seed in EVALUATION_SEEDS:
                descriptors = heldout[(domain, int(evaluation_seed))]
                audit = descriptors[:POLICY_COUNT]
                expected_indices = list(range(POLICY_COUNT))
                ...
                for training_seed in TRAINING_SEEDS:
                    ...
                    if [int(x) for x in cell["policy_state_indices"]] != expected_indices:
                    ...
                    recomputed = np.asarray(
                        policy.batch_probabilities(
                            [item.observation_v3 for item in audit],
                            [item.legal_slots for item in audit],
                        ),
                        dtype=np.float64,
                    )
                    ...
                    if max_abs > TOL:
                        raise RuntimeError(
                            f"worker policy recomputation mismatch {rep}/{domain}/{training_seed}/{evaluation_seed}: {max_abs}"
                        )
                    stored_rows.append(stored.tolist())
                    recomputed_rows.append(recomputed.tolist())
                    seed_details.append({
                        "training_seed": int(training_seed),
                        "max_abs_worker_vs_recomputed": max_abs,
                        "artifact_path": str(policy.artifact_path),
                    })
                ...
                for name in ("mean", "p50", "p95", "max"):
                    _assert_close(stored_metric[name], recomputed_metric[name], f"stored/recomputed {name}", tol=TOL)
                ...
                for name in ("mean", "p95", "max"):
                    _assert_close(stored_metric[name], expected[name], f"blocker evidence {name}")
                ...
                results.append({
                    ...
                    "heldout_first1024_payload_sha256": descriptor_sha,
                    "legal_slots_first1024_sha256": legal_sha,
                    "action_width": NUM_ACTIONS,
                    "seed_details": seed_details,
                    "cross_seed_policy_stability": stored_metric,
                })
