...
SCHEMA = "SPINCORE_R7_5_3C_UPSTREAM_RNG_FACTORIAL_FINAL_CELL_V1"
...
def _train_fixed_policy(bundle, memory, representation: str) -> list[float]:
    if not memory.items:
        raise RuntimeError("empty upstream-factorial strategy memory")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fixed final-policy readout for upstream RNG factorial")
    ...
    source_bundle, progress, _spec, extra = load_representation_v3_checkpoint(
        ...
        expected_seed=int(args.learning_seed),
    ...
    if progress.phase != "post_advantage_fit" or int(progress.iteration) != 3:
        raise RuntimeError("source checkpoint must be the completed iteration-3 pre-policy-fit diagnostic checkpoint")
    state = dict(extra.get("stage_state") or {})
    if int(state.get("diagnostic_sampling_seed", -1)) != int(args.sampling_seed):
        raise RuntimeError("source diagnostic sampling-seed mismatch")
    if int(state.get("diagnostic_learning_seed", -1)) != int(args.learning_seed):
        raise RuntimeError("source diagnostic learning-seed mismatch")
    ...
    policy = FinalizedV3Policy(
        ...
        artifact_path="UPSTREAM_RNG_FACTORIAL_FIXED_READOUT",
    )
