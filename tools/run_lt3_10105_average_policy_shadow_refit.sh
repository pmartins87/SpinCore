...
"${PY}" -m py_compile tools/audit_lt3_10105_average_policy_shadow_refit.py

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN="${ROOT}/runs/lt3_10105_average_policy_shadow_refit/${STAMP}"
mkdir -p "${RUN}"
REPORT="${RUN}/average_policy_shadow_refit_10105.json"

"${PY}" tools/audit_lt3_10105_average_policy_shadow_refit.py \
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  if [[ -n "${DESKTOP_WIN}" ]]; then
    ...
    if [[ -n "${DESKTOP_WSL}" && -d "${DESKTOP_WSL}" ]]; then
      cp -f "${REPORT}" "${DESKTOP_WSL}/SpinCore_LT3_10105_average_policy_shadow_refit.json"
      echo "desktop_report=${DESKTOP_WIN}\\SpinCore_LT3_10105_average_policy_shadow_refit.json"
    fi
  fi
fi
echo "LT3_10105_AVERAGE_POLICY_SHADOW_REFIT_RUN_COMPLETE"
