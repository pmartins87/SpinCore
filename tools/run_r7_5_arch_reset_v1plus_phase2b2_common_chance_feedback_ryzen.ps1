...
if ($Dirty) {
    throw "Tracked worktree is dirty. Commit/revert tracked changes before Phase2B2.`n$Dirty"
}

$Precommit = Join-Path $Repo 'validation/R7_5_ARCH_RESET_V1PLUS_PHASE2B2_COMMON_CHANCE_FEEDBACK_PRECOMMIT_20260822.md'
...
foreach ($Path in @($Precommit, $Phase2B1Evidence, $Phase2B0Evidence, $Phase2AEvidence)) {
    if (-not (Test-Path $Path -PathType Leaf)) { throw "Missing frozen Phase2B2 contract/evidence: $Path" }
}
...
if ($LASTEXITCODE -ne 0) { throw 'Frozen Phase2B2 Python/Torch/Numpy runtime verification failed.' }
...
$Tool = Join-Path $Repo 'tools/r7_5_arch_reset_v1plus_phase2b2_common_chance_feedback.py'
$Test = Join-Path $Repo 'tools/test_r7_5_arch_reset_v1plus_phase2b2_common_chance_feedback.py'
Write-Host '[V1+ Phase2B2] compiling diagnostic scripts...'
& $Python -m py_compile $Tool $Test
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 py_compile failed.' }
Write-Host '[V1+ Phase2B2] running deterministic synthetic tests...'
& $Python $Test
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 synthetic tests failed.' }

$Phase2B1 = Join-Path $Repo 'ryzen_v1plus_phase2b1/R7_5_ARCH_RESET_V1PLUS_PHASE2B1_TARGET_VARIANCE.json'
if (-not (Test-Path $Phase2B1 -PathType Leaf)) {
    throw "Missing local completed Phase2B1 result: $Phase2B1"
}
Write-Host '[V1+ Phase2B2] validating exact Phase2B1 local evidence identity...'
& $Python -c "import hashlib,json,sys; p=sys.argv[1]; e=json.load(open(sys.argv[2],encoding='utf-8')); raw=open(p,'rb').read(); d=json.loads(raw); h=hashlib.sha256(raw).hexdigest(); assert h==e['uploaded_result_sha256'],(h,e['uploaded_result_sha256']); assert d['schema']=='SPINCORE_R7_5_ARCH_RESET_V1PLUS_PHASE2B1_TARGET_VARIANCE_V1'; assert d['status']=='PHASE2B1_K4_SCREEN_FAIL_NO_GENERIC_K4_TRAINING_PILOT'; assert d['decision']['source_classification']=='CHANCE_DOMINANT'; assert d['decision']['small_causal_pilot_precommit_allowed'] is False; assert len(d['collision_groups'])==15; assert all(len(g['deck_seeds'])==16 for g in d['collision_groups']); print('Phase2B1 exact local result/evidence PASS',h)" $Phase2B1 $Phase2B1Evidence
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 Phase2B1 local evidence preflight failed.' }

Write-Host '[V1+ Phase2B2] validating authoritative H2/3H source/model contract...'
...
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 source/model contract preflight failed.' }

$Build = Join-Path $Repo 'build_phase2b2'
if (Test-Path $Build) { Remove-Item -Recurse -Force $Build }
& cmake -S $Repo -B $Build -G 'Visual Studio 17 2022' -A x64
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 CMake x64 configure failed.' }
& cmake --build $Build --config Release --target spincore_solver_c --parallel
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 solver x64 build failed.' }
$SolverCandidates = @(
    @(
...
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 solver PE architecture preflight failed.' }
...
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 solver ABI/SPNNIV3 preflight failed.' }
...
Write-Host '[V1+ Phase2B2] validating both frozen Phase2A source behavior ensembles...'
& $Python -c "import sys,torch; from pathlib import Path; root=Path(sys.argv[1]); sha=sys.argv[2]; seeds=(1342191342,1801739323); rows=[];`nfor s in seeds:`n q=torch.load(root/f'seed_{s}'/'resume_checkpoint.pt',map_location='cpu',weights_only=False); e=q.get('extra',{}); assert q.get('execution_sha')==sha and int(q.get('seed',-1))==s; assert q.get('representation')=='H2_RELATIONAL_EXACT_STRUCTURED_HISTORY_FINAL' and q.get('domain')=='THREE_HANDED'; assert q.get('progress',{}).get('phase')=='phase2a_resume' and int(q.get('progress',{}).get('global_root',-1))==768; assert int(e.get('stage_index',-1))==12 and len(e.get('behavior_model_states') or [])==4; rows.append((s,len(e['behavior_model_states'])));`nprint('Phase2B2 source ensemble identity PASS',rows)" $Input $SourceExecutionSha
if ($LASTEXITCODE -ne 0) { throw 'Phase2B2 Phase2A ensemble identity preflight failed.' }

$Output = Join-Path $Repo 'ryzen_v1plus_phase2b2'
$Result = Join-Path $Output 'R7_5_ARCH_RESET_V1PLUS_PHASE2B2_COMMON_CHANCE_FEEDBACK.json'
New-Item -ItemType Directory -Force -Path $Output | Out-Null
$Workers = 12

Write-Host "[V1+ Phase2B2] diagnostic HEAD: $Head"
Write-Host "[V1+ Phase2B2] source execution SHA: $SourceExecutionSha"
Write-Host "[V1+ Phase2B2] solver: $Solver"
Write-Host "[V1+ Phase2B2] workers: $Workers independent processes x 1 Torch/OMP/MKL thread"
Write-Host '[V1+ Phase2B2] exact work: 15 stored collision groups x 16 stored deals x 2 source behaviors x 2 traversal-RNG coupling arms = 960 traversals.'
Write-Host '[V1+ Phase2B2] FRESH SOLVER TRAVERSAL ONLY; no reservoir insertion, optimizer step, model fit, or checkpoint mutation.'

& $Python (Join-Path $Repo 'tools/spincore_ryzen_frozen_runner.py') `
    --expected-commit $Head `
    --run-name 'r7_5_arch_reset_v1plus_phase2b2_common_chance_feedback' `
    --contract 'validation/R7_5_ARCH_RESET_V1PLUS_PHASE2B2_COMMON_CHANCE_FEEDBACK_PRECOMMIT_20260822.md' `
    ...
    --contract 'tools/test_r7_5_arch_reset_v1plus_phase2b2_common_chance_feedback.py' `
...
if ($LASTEXITCODE -ne 0) {
    throw "Phase2B2 failed with exit code $LASTEXITCODE. Preserve Phase2A and Phase2B1 artifacts; do not start a training run."
}
if (-not (Test-Path $Result -PathType Leaf)) { throw 'Phase2B2 returned success without result JSON.' }
$Hash = (Get-FileHash -Algorithm SHA256 $Result).Hash.ToLowerInvariant()
Write-Host ''
Write-Host '[V1+ Phase2B2] COMPLETE'
Write-Host "[V1+ Phase2B2] result: $Result"
Write-Host "[V1+ Phase2B2] SHA256: $Hash"
Write-Host '[V1+ Phase2B2] No training was performed. Send the result JSON back for the next causal decision.'
