// Authoritative R7.5.4 structural-audit executor V3.
// Reuses the already regression-compiled V1 implementation and changes only
// the pre-output-corrected seed stream / trajectory count / evidence schema.
...
void audit_domain_v3(
    ...
    for (auto evaluation_seed : evaluation_seeds) {
        for (std::size_t scenario_index = 0; scenario_index < scenarios.size(); ++scenario_index) {
            for (std::uint64_t trajectory_index = 0; trajectory_index < 96; ++trajectory_index) {
                ++stats.trajectories;
...
int main(int argc, char** argv) {
    ...
    DomainStats three;
    try {
        audit_domain_v3(hu_scenarios(), hu);
        audit_domain_v3(three_handed_scenarios(), three);
    ...
    json << "{";
    json << "\"schema\":\"SPINCORE_R7_5_4_ACTION_STRUCTURAL_AUDIT_V3\",";
    json << "\"freeze_schema\":\"SPINCORE_R7_5_4_STRUCTURAL_AUDIT_FREEZE_V3\",";
    ...
    json << "\"trajectories_per_scenario_seed\":96,";
