...
GATE_REPORT_SHA="fd24a61804f28db5cd94ea41372c4163cfe028a58d3d703abdb0d1e75ff9dbdd"

SOURCE_ITERATION=8200
TARGET_ITERATION=9250
ADDITIONAL_ITERATIONS=1050
MILESTONE_ITERATION=8600
ROOTS_PER_ITERATION=600
EXPECTED_NEW_ROOTS=630000
EXPECTED_TOTAL_ROOTS=5550000
WORKERS=31
THREADS=8
...
[ "$(sha256sum "$GATE_REPORT"|awk '{print $1}')" = "$GATE_REPORT_SHA" ] || {
  echo "ERROR: 8200 end-to-end gate report SHA mismatch" >&2; exit 6;
}
...
assert d["recommended_24h"]["start_iteration"]==8200
assert d["recommended_24h"]["additional_iterations"]==1050
assert d["recommended_24h"]["target_iteration"]==9250
assert d["holdout_touched"] is False
print("LT3_24H_GATE_CONTRACT_PASS")
PY
...
if [ -n "$HOST_C_KIB" ] && [ "$HOST_C_KIB" -lt $((20*1024*1024)) ]; then
  ...
  echo "The 8600 preserved milestone plus active/atomic checkpoints need headroom." >&2
  exit 12
fi
...
echo "LT3_PARALLEL_24H_PREFLIGHT_PASS"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="$ROOT/runs/lt3_parallel_8200_9250/$STAMP"
mkdir -p "$RUN_DIR/work" "$RUN_DIR/milestones"

CHECKPOINT="$RUN_DIR/checkpoint.pt"
ENSEMBLE="$RUN_DIR/hu_ensemble_state.pt"
REPORT="$RUN_DIR/lt3_parallel_8200_9250.json"
LOG="$RUN_DIR/training.log"
MEMLOG="$RUN_DIR/memory.log"
PROVENANCE="$RUN_DIR/provenance.txt"

cat > "$PROVENANCE" <<EOF
schema=SPINCORE_LT3_PARALLEL_8200_9250_PROVENANCE_V1
...
hu_fit_execution=process_parallel_4x8_exact_parity
workers=$WORKERS
torch_threads_parent=$THREADS
parallel_concurrency=$PARALLEL_CONCURRENCY
parallel_member_threads=$PARALLEL_MEMBER_THREADS
checkpoint_every=$CHECKPOINT_EVERY
hu_preflop_board_average_k=1
projected_hours_from_gate=24.34621249145194
...
copy_evidence() {
  ...
  [ -d "$dest" ] || return 0
  [ -f "$REPORT" ] && cp "$REPORT" "$dest/SpinCore_LT3_parallel_8200_9250.json" || true
  [ -f "$PROVENANCE" ] && cp "$PROVENANCE" "$dest/SpinCore_LT3_parallel_8200_9250_provenance.txt" || true
  [ -f "$MEMLOG" ] && cp "$MEMLOG" "$dest/SpinCore_LT3_parallel_8200_9250_memory.log" || true
}

echo "=== SpinCore LT3 exact-parity parallel continuation ==="
echo "source=durable LT3@8200 checkpoint+sidecar"
echo "target=9250 / +1050 iterations / +630,000 roots"
echo "expected_duration_from_gate≈24.35h"
echo "3H=fresh100; HU=ENS8 8xfresh400 executed 4x8 exact-parity"
echo "milestone 8600 will be preserved automatically"
echo "checkpoint every 50 iterations"
...
if [ "$STATUS" -ne 0 ]; then
  copy_evidence
  echo "LT3_PARALLEL_8200_9250_TRAINING_FAIL status=$STATUS" >&2
  echo "run_dir=$RUN_DIR" >&2
  echo "Do not restart automatically. Send training.log/report to ChatGPT." >&2
  exit "$STATUS"
fi

"$PY" - "$REPORT" "$CHECKPOINT" "$ENSEMBLE" "$RUN_DIR/milestones" <<'PY'
import json,sys,torch
from pathlib import Path
report,checkpoint,ensemble,milestones=map(Path,sys.argv[1:5])
d=json.loads(report.read_text())
assert d["schema"]=="SPINCORE_LT3_PARALLEL_CONTINUATION_8200_9250_V1"
assert d["status"]=="PASS"
assert d["source_iteration"]==8200
assert d["target_iteration"]==9250
assert d["completed_iteration"]==9250
assert d["source_unchanged"] is True
assert d["intervention"]["poker_or_training_algorithm_change"] is False
assert d["intervention"]["parallel_fit_layout"]=="4x8"
assert d["method"]["new_training_roots"]==630000
...
assert int(e["completed_iteration"])==9250
assert int(e["ensemble_size"])==8 and int(e["member_steps"])==400
final=d["final"]["domains"]
assert sum(int(v["roots"]) for v in final.values())==5550000
print("LT3_PARALLEL_8200_9250_POSTVALIDATION_PASS")
...
print("milestone_8600_checkpoint="+m["checkpoint"])
PY

copy_evidence

# The mmap mirror is only training infrastructure. Remove it after the final
# checkpoint/report are safely written.
rm -rf "$RUN_DIR/work/packed_hu_adv" || true

echo
echo "LT3_PARALLEL_8200_9250_TRAINING_PASS"
echo "source_8200_unchanged=true"
echo "milestone_8600_preserved=true"
echo "target_9250_complete=true"
echo "lt2_final_holdout_untouched=true"
echo "lt3_holdout_untouched=true"
echo "run_dir=$RUN_DIR"
echo "report=$REPORT"
echo "STOP HERE. Send SpinCore_LT3_parallel_8200_9250.json to ChatGPT."
