...
WORKERS="${SPINCORE_POLICY_CHAIN_WORKERS:-16}"
SCENARIOS="${SPINCORE_POLICY_CHAIN_SCENARIOS_PER_SEED:-5000}"
...
RUN_DIR="$ROOT/runs/lt2_hu_policy_chain/$STAMP"
mkdir -p "$RUN_DIR"
REPORT="$RUN_DIR/hu_policy_chain.json"
...
printf "=== SpinCore LT2 HU policy-chain audit ===\n"
...
printf "policies=AVG_A,AVG_B,BEH_A,BEH_B; baselines=UNIFORM_LEGAL,PASSIVE_CALLER,JAMMER\n\n"

"$PYTHON_RUN" tools/audit_lt2_hu_policy_chain.py \
  --solver "$SOLVER" \
  --stage-a "$STAGE_A" \
  --stage-b "$STAGE_B" \
...
assert d["schema"] == "SPINCORE_LT2_HU_POLICY_CHAIN_EVAL_V1"
...
for baseline, block in d["summary"].items():
    assert block["clusters"] > 0
    for field in ("avg_b_minus_a","beh_b_minus_a","chain_delta_b_minus_a"):
        assert math.isfinite(float(block[field]["mean"]))
print("LT2_HU_POLICY_CHAIN_EVAL_POSTVALIDATION_PASS")
PY
...
if [ "$AFTER_A" != "$STAGE_A_SHA" ] || [ "$AFTER_B" != "$STAGE_B_SHA" ]; then
  echo "ERROR: source checkpoint changed during policy-chain audit" >&2
  exit 7
fi

DEST="/mnt/c/Users/Rz9/Downloads"
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_hu_policy_chain.json"
fi
...
printf "report=%s\n" "$REPORT"
printf "STOP HERE. Do not train or launch another diagnostic until the policy-chain result is reviewed.\n"
