...
WORKERS="${SPINCORE_BEH_FIRST_DIV_WORKERS:-16}"
SCENARIOS="${SPINCORE_BEH_FIRST_DIV_SCENARIOS_PER_SEED:-5000}"
...
RUN_DIR="$ROOT/runs/lt2_hu_behavior_first_divergence/$STAMP"
mkdir -p "$RUN_DIR"
REPORT="$RUN_DIR/hu_behavior_first_divergence.json"
...
printf "=== SpinCore LT2 HU current-behavior first-divergence forensic ===\n"
...
"$PYTHON_RUN" tools/audit_lt2_hu_behavior_first_divergence.py \
  --solver "$SOLVER" \
  --stage-a "$A_SOURCE" \
  --stage-b "$B_SOURCE" \
...
print("LT2_HU_BEHAVIOR_FIRST_DIVERGENCE_POSTVALIDATION_PASS")
PY
...
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_hu_behavior_first_divergence.json"
fi
...
printf "report=%s\n" "$REPORT"
printf "STOP HERE. Do not train until the current-behavior localization is reviewed.\n"
