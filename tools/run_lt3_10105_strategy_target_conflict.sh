...
"${PY}" -m py_compile tools/audit_lt3_10105_strategy_target_conflict.py

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN="${ROOT}/runs/lt3_10105_strategy_target_conflict/${STAMP}"
mkdir -p "${RUN}"
REPORT="${RUN}/strategy_target_conflict_10105.json"

"${PY}" tools/audit_lt3_10105_strategy_target_conflict.py \
  --checkpoint "${CHECKPOINT}" \
  --report "${REPORT}" \
  --selected-groups 20000 \
  --reps-per-group 4
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  if [[ -n "${DESKTOP_WIN}" ]]; then
    ...
    if [[ -n "${DESKTOP_WSL}" && -d "${DESKTOP_WSL}" ]]; then
      cp -f "${REPORT}" "${DESKTOP_WSL}/SpinCore_LT3_10105_strategy_target_conflict.json"
      echo "desktop_report=${DESKTOP_WIN}\\SpinCore_LT3_10105_strategy_target_conflict.json"
    fi
  fi
fi
echo "LT3_10105_STRATEGY_TARGET_CONFLICT_RUN_COMPLETE"
