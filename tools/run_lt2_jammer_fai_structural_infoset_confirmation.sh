...
RUN_DIR="$ROOT/runs/lt2_jammer_fai_structural_infoset_confirmation/$STAMP"
mkdir -p "$RUN_DIR"
REPORT="$RUN_DIR/jammer_fai_structural_infoset_confirmation.json"
...
printf "=== SpinCore LT2 Jammer FAI structural infoset confirmation ===\n"
...
printf "focus=legal {FOLD,CALL}, one public action before FAI\n"
printf "selection=fold-mass shift sign only inside frozen structure; NO action/outcome/Q selection\n"
printf "groups=32 B_MORE_FOLD + 32 B_LESS_FOLD per seed; total=384\n"
printf "reference=64 uniform compatible hands x 8 future boards = 512 deals/anchor\n\n"

"$PYTHON_RUN" tools/audit_lt2_jammer_fai_structural_infoset_confirmation.py \
  --solver "$SOLVER" \
  --stage-a "$A_SOURCE" \
  --stage-b "$B_SOURCE" \
  --scenarios-per-seed 5000 \
  --anchors-per-shift-per-seed 32 \
  --reference-hands 64 \
...
assert d["schema"] == "SPINCORE_LT2_JAMMER_FAI_STRUCTURAL_INFOSET_CONFIRMATION_V1"
...
assert m["structural_focus"]["legal_slots"] == [0,1]
assert m["structural_focus"]["common_public_action_count"] == 1
...
print("LT2_JAMMER_FAI_STRUCTURAL_INFOSET_CONFIRMATION_POSTVALIDATION_PASS")
PY
...
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_jammer_fai_structural_infoset_confirmation.json"
fi
...
printf "report=%s\n" "$REPORT"
printf "STOP HERE. Do not train until the structural infoset confirmation is reviewed.\n"
