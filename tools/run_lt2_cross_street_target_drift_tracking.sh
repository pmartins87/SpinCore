...
THREADS="${SPINCORE_DRIFT_THREADS:-8}"
STATES="${SPINCORE_DRIFT_STATES_PER_SEED_STATUS:-2}"
BOARDS="${SPINCORE_DRIFT_FUTURE_BOARDS:-8}"
REPEATS="${SPINCORE_DRIFT_REPEATS_PER_BOARD:-4}"
...
RUN_DIR="$ROOT/runs/lt2_cross_street_target_drift_tracking/$STAMP"
mkdir -p "$RUN_DIR"
REPORT="$RUN_DIR/cross_street_target_drift_tracking.json"

export PYTHONPATH="$ROOT/python:$ROOT/tools"
export OMP_NUM_THREADS=1
export MKL_NUM_THREADS=1

printf "=== SpinCore LT2 Stage-A/B target-drift + model-tracking audit ===\n"
printf "mode=READ ONLY; NO TRAINING ROOTS; NO OPTIMIZER STEPS\n"
printf "same FAILURE/CONTROL contexts as previous cross-street audit\n"
printf "forensic seeds=20260920..20260925; holdout 20261001..20261006 untouched\n"
printf "states/seed/status=%s total=%s\n" "$STATES" "$((3 * 2 * 6 * STATES))"
printf "paired Stage-A/B conditional references=%s future boards x %s repeats, exact1\n" "$BOARDS" "$REPEATS"
printf "canonical action-gap gauge used for all target/model comparisons\n\n"

"$PYTHON_RUN" tools/audit_lt2_cross_street_target_drift_tracking.py \
  --solver "$SOLVER" \
  --stage-a "$STAGE_A" \
  --stage-b "$STAGE_B" \
  ...
  --repeats-per-board "$REPEATS" \
  --exact-opponent-levels 1 \
  --threads "$THREADS" \
  --report "$REPORT"
...
assert d["schema"] == "SPINCORE_LT2_CROSS_STREET_TARGET_DRIFT_TRACKING_V1"
...
assert m["selection_reused_from_cross_street_future_chance_v1"] is True
for group in m["groups"]:
    for status in ("FAILURE","CONTROL"):
        s=d["summaries"][group][status]
        assert s["n"] == int(m["states_per_seed_status"]) * 6
        for field in ("target_drift_mse","stage_a_own_target_model_mse","stage_b_own_target_model_mse","tracking_error_mse"):
            assert math.isfinite(float(s["metrics"][field]["mean"]))
print("LT2_CROSS_STREET_TARGET_DRIFT_TRACKING_POSTVALIDATION_PASS")
PY
...
if [ "$AFTER_A" != "$STAGE_A_SHA" ] || [ "$AFTER_B" != "$STAGE_B_SHA" ]; then
  echo "ERROR: source checkpoint changed during target-drift audit" >&2
  exit 7
fi

DEST="/mnt/c/Users/Rz9/Downloads"
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_cross_street_target_drift_tracking.json"
fi
...
printf "report=%s\n" "$REPORT"
printf "STOP HERE. Do not train or launch another diagnostic until this matrix is reviewed.\n"
