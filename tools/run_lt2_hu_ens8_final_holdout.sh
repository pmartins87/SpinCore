...
"$PY" -m py_compile tools/audit_lt2_hu_ens8_final_holdout.py
echo "PYTHON_PREFLIGHT_PASS"

STAMP="$(date +%Y%m%d_%H%M%S)"
DIR="$ROOT/runs/lt2_hu_ens8_final_holdout/$STAMP"
mkdir -p "$DIR"
REPORT="$DIR/hu_ens8_final_holdout.json"
...
echo "=== SpinCore LT2 HU ENS8 FINAL SEALED HOLDOUT ==="
echo "candidate=ENS8 current behavior @8100"
echo "holdout seeds=20261001..20261006"
echo "criteria frozen in docs/LT2_HU_ENS8_FINAL_HOLDOUT_PROTOCOL_20260921.md"
echo "git_commit=$GIT_COMMIT"
echo "NO TRAINING; ONE COMPLETED HOLDOUT RUN IS FINAL"

"$PY" tools/audit_lt2_hu_ens8_final_holdout.py   --solver "$SOLVER"   --stage-7600 "$P7600"   --stage-8000 "$P8000"   --stage-8100 "$P8100"   --ensemble-8100 "$E8100"   --scenarios-per-seed 6000   --workers 31   --threads-fit 8   --git-commit "$GIT_COMMIT"   --report "$REPORT"

"$PY" - "$REPORT" <<'PY'
import json,sys
from pathlib import Path
d=json.loads(Path(sys.argv[1]).read_text())
assert d["schema"]=="SPINCORE_LT2_HU_ENS8_FINAL_HOLDOUT_V1"
assert d["verdict"] in {"PASS","FAIL"}
m=d["method"]
assert m["holdout_seeds"]==[20261001,20261002,20261003,20261004,20261005,20261006]
assert m["read_only_source_artifacts"] is True
assert m["new_training_roots"]==m["source_training_memory_writes"]==0
assert m["no_posthoc_tuning"] is True
assert len(d["criteria"])==11
print("LT2_HU_ENS8_FINAL_HOLDOUT_POSTVALIDATION_PASS")
print("STRATEGIC_VERDICT="+d["verdict"])
PY
...
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_hu_ens8_final_holdout.json"
fi

echo "LT2_HU_ENS8_FINAL_HOLDOUT_COMPLETE"
echo "STOP. Do not rerun or tune from this holdout. Send SpinCore_LT2_hu_ens8_final_holdout.json to ChatGPT."
