...
from spincore.deepcrusher_native_symbols import DeepCrusherPrimitiveSymbols  # noqa: E402
...
def _classify(name: str) -> str:
    ...
    if low.startswith("pt_"):
        return "pokertracker"
    if low.startswith(("network$", "chair$", "log$")) or low in {"prwin", "prtie", "colourcode_dealerchair", "colourcode_headsupchair"}:
        return "environment"
...
def main() -> int:
    ...
    environment = {name.lower() for name in closure.environment_identifiers}
    supported = sorted(
        (name for name in native if name.lower() in supported_folded),
        key=str.lower,
    )
    unresolved = sorted(
        (name for name in native if name.lower() not in supported_folded),
        key=str.lower,
    )
