...
"$PY" -m py_compile tools/audit_lt2_hu_ens8_post_pilot_forensic.py
echo "PYTHON_PREFLIGHT_PASS"

STAMP="$(date +%Y%m%d_%H%M%S)"
DIR="$ROOT/runs/lt2_hu_ens8_post_pilot_forensic/$STAMP"
mkdir -p "$DIR"
REPORT="$DIR/hu_ens8_post_pilot_forensic.json"

P8100_SHA="$(sha256sum "$P8100"|awk '{print $1}')"
E8100_SHA="$(sha256sum "$E8100"|awk '{print $1}')"
echo "pilot_checkpoint_sha256=$P8100_SHA"
echo "pilot_ensemble_sha256=$E8100_SHA"

export SPINCORE_TORCH_THREADS=1
export OMP_NUM_THREADS=1
export MKL_NUM_THREADS=1

echo "=== SpinCore LT2 HU ENS8 post-pilot forensic ==="
echo "rebuild source ENS8_A @8000; compare current ENS8 8000/8100 + AveragePolicy 8000/8100"
echo "READ ONLY sources; NO CFR ROOTS; HOLDOUT UNTOUCHED"

"$PY" tools/audit_lt2_hu_ens8_post_pilot_forensic.py   --solver "$SOLVER"   --stage-7600 "$P7600"   --stage-8000 "$P8000"   --stage-8100 "$P8100"   --ensemble-8100 "$E8100"   --scenarios-per-seed 5000   --workers 31   --threads-fit 8   --report "$REPORT"

"$PY" - "$REPORT" <<'PY'
import json,sys
from pathlib import Path
d=json.loads(Path(sys.argv[1]).read_text())
assert d["schema"]=="SPINCORE_LT2_HU_ENS8_POST_PILOT_FORENSIC_V1"
m=d["method"]
...
assert d["root_drift_8100_minus_8000"]["root_count"]==sum(m["hu_scenarios_by_seed"].values())
assert set(d["ev"])=={"UNIFORM_LEGAL","PASSIVE_CALLER","JAMMER"}
print("LT2_HU_ENS8_POST_PILOT_FORENSIC_POSTVALIDATION_PASS")
PY
...
if [ -d "$DEST" ]; then
  cp "$REPORT" "$DEST/SpinCore_LT2_hu_ens8_post_pilot_forensic.json"
fi

echo "LT2_HU_ENS8_POST_PILOT_FORENSIC_PASS"
echo "STOP HERE. Send SpinCore_LT2_hu_ens8_post_pilot_forensic.json to ChatGPT."
