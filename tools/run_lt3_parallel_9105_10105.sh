...
SOURCE_DIR="$ROOT/runs/lt3_parallel_8200_9105/20260921_131118"
SOURCE_CHECKPOINT="$SOURCE_DIR/checkpoint.pt"
SOURCE_ENSEMBLE="$SOURCE_DIR/hu_ensemble_state.pt"
SOURCE_CHECKPOINT_SHA="21945e27c43c7e6c1cdb77018cd66dc90b3fab72c29a9034bb4a9f97cc0e6c68"
SOURCE_ENSEMBLE_SHA="b9c3ffffc7139eeb77c4b4182136e10ada5cad2f023aa6e560e164e2e0ac9256"
...
EXPECTED_NEW_ROOTS=600000
EXPECTED_TOTAL_ROOTS=6063000
WORKERS=31
THREADS=8
PARALLEL_CONCURRENCY=4
PARALLEL_MEMBER_THREADS=8
CHECKPOINT_EVERY=50
PROJECTED_HOURS=23.2
...
[ "$(sha256sum "$SOURCE_CHECKPOINT"|awk '{print $1}')" = "$SOURCE_CHECKPOINT_SHA" ] || {
  echo "ERROR: frozen LT3@9105 checkpoint SHA mismatch" >&2; exit 4;
}
[ "$(sha256sum "$SOURCE_ENSEMBLE"|awk '{print $1}')" = "$SOURCE_ENSEMBLE_SHA" ] || {
  echo "ERROR: frozen LT3@9105 ensemble SHA mismatch" >&2; exit 5;
}

"$PY" - "$GATE_REPORT" "$SOURCE_CHECKPOINT" "$SOURCE_ENSEMBLE" <<'PY'
import json,sys,torch
from pathlib import Path
gate,cp,ens=map(Path,sys.argv[1:4])
...
assert int(p["completed_iteration"])==9105 and bool(p["finalized"]) is True
assert int(e["completed_iteration"])==9105
assert int(e["ensemble_size"])==8 and int(e["member_steps"])==400
print("LT3_9105_10105_GATE_CONTRACT_PASS")
PY

if pgrep -af "run_lt3_heavy_ens8_h1.py|run_lt3_parallel_ens8_continuation.py|run_lt3_parallel_9105_10105.py" >/dev/null 2>&1; then
  echo "ERROR: an LT3 long trainer is already running." >&2
  pgrep -af "run_lt3_heavy_ens8_h1.py|run_lt3_parallel_ens8_continuation.py|run_lt3_parallel_9105_10105.py" >&2 || true
  exit 7
fi
...
"$PY" -m py_compile   python/spincore_nn/reservoir.py   tools/lt3_hu_ens8_parallel_fit.py   tools/lt3_parallel_continuation_core.py   tools/run_lt3_parallel_9105_10105.py

"$PY" python_tests/test_reservoir_write_observer.py
"$PY" python_tests/test_lean_action_policy_ensemble.py
echo "LT3_PARALLEL_9105_10105_PREFLIGHT_PASS"

STAMP="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="$ROOT/runs/lt3_parallel_9105_10105/$STAMP"
mkdir -p "$RUN_DIR/work" "$RUN_DIR/milestones"

CHECKPOINT="$RUN_DIR/checkpoint.pt"
ENSEMBLE="$RUN_DIR/hu_ensemble_state.pt"
REPORT="$RUN_DIR/lt3_parallel_9105_10105.json"
...
copy_evidence() {
  ...
  [ -d "$dest" ] || return 0
  [ -f "$REPORT" ] && cp "$REPORT" "$dest/SpinCore_LT3_parallel_9105_10105.json" || true
  [ -f "$MEMLOG" ] && cp "$MEMLOG" "$dest/SpinCore_LT3_parallel_9105_10105_memory.log" || true
}

echo "=== SpinCore LT3 research continuation (~23 h) ==="
echo "source=frozen finalized LT3@9105 checkpoint+ENS8 sidecar"
echo "target=10105 / +1000 iterations / +600,000 roots"
echo "projected_duration≈$PROJECTED_HOURS h"
echo "3H=fresh100; HU=ENS8 8xfresh400 executed 4x8 exact-parity"
echo "raw milestone 9600 will be preserved automatically"
echo "checkpoint every 50 iterations plus final 10105"
echo "no sealed holdout access"
...
[ "$(sha256sum "$SOURCE_CHECKPOINT"|awk '{print $1}')" = "$SOURCE_CHECKPOINT_SHA" ] || {
  copy_evidence
  echo "ERROR: source 9105 checkpoint changed" >&2; exit 13;
}
[ "$(sha256sum "$SOURCE_ENSEMBLE"|awk '{print $1}')" = "$SOURCE_ENSEMBLE_SHA" ] || {
  copy_evidence
  echo "ERROR: source 9105 ensemble changed" >&2; exit 14;
}

if [ "$STATUS" -ne 0 ]; then
  copy_evidence
  echo "LT3_PARALLEL_9105_10105_TRAINING_FAIL status=$STATUS" >&2
...
assert d["schema"]=="SPINCORE_LT3_PARALLEL_CONTINUATION_9105_10105_V1"
assert d["status"]=="PASS"
assert d["source_iteration"]==9105
assert d["target_iteration"]==10105
assert d["completed_iteration"]==10105
assert d["source_finalized"] is True
assert d["source_unchanged"] is True
assert d["intervention"]["parallel_fit_layout"]=="4x8"
assert d["method"]["new_training_roots"]==600000
assert d["holdout_touched"] is False
assert checkpoint.is_file() and ensemble.is_file()
m=d["milestone_files"]
assert int(m["iteration"])==9600
...
assert int(e["completed_iteration"])==10105
assert int(e["ensemble_size"])==8 and int(e["member_steps"])==400
final=d["final"]["domains"]
assert sum(int(v["roots"]) for v in final.values())==6063000
print("LT3_PARALLEL_9105_10105_POSTVALIDATION_PASS")
...
echo
echo "LT3_PARALLEL_9105_10105_TRAINING_PASS"
echo "source_9105_unchanged=true"
echo "milestone_9600_preserved=true"
echo "target_10105_complete=true"
echo "run_dir=$RUN_DIR"
echo "report=$REPORT"
echo "STOP HERE. Send SpinCore_LT3_parallel_9105_10105.json to ChatGPT."
