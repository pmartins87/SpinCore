...
def main() -> int:
    parser = argparse.ArgumentParser(description="Hard stability readout for R7.5.3C x4 chance coverage")
    ...
    if evidence.get("schema") != "SPINCORE_R7_5_3C_SAMPLING_SPLIT_EVIDENCE_V1":
        raise RuntimeError("wrong source diagnostic evidence schema")
    if evidence.get("classification") != "DECK_CHANCE_DOMINANT":
        raise RuntimeError("x4 readmission requires DECK_CHANCE_DOMINANT source diagnosis")
    ...
    result = {
        ...
        "purpose": "Winner-independent hard stability readmission after x4 independent chance coverage. This is not representation selection and does not replace the frozen full strategic Phase 2 evaluation.",
        ...
        "next_action_if_blocked": "Remain BLOCKED; do not relax gates. Freeze a further winner-independent chance-coverage stratification or larger-coverage experiment based on this result.",
