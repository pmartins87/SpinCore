...
RUN="${ROOT}/runs/3h_average_policy_semantic_continuation/${STAMP}"
mkdir -p "${RUN}"
BUNDLE="${RUN}/spincore_hybrid_10105.pt"
MODEL="${RUN}/3h_average_policy_semantic_continuation_model.pt"
REPORT="${RUN}/3h_average_policy_semantic_continuation.json"
...
"${PY}" tools/audit_3h_average_policy_semantic_continuation_10105.py \
  --checkpoint "${CHECKPOINT}" \
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  if [[ -n "${DESKTOP_WIN}" ]]; then
    ...
    if [[ -n "${DESKTOP_WSL}" && -d "${DESKTOP_WSL}" ]]; then
      cp -f "${REPORT}" "${DESKTOP_WSL}/SpinCore_3H_average_policy_semantic_continuation.json"
      echo "desktop_report=${DESKTOP_WIN}\\SpinCore_3H_average_policy_semantic_continuation.json"
    fi
  fi
fi

echo "3H_AVERAGE_POLICY_SEMANTIC_CONTINUATION_RUN_COMPLETE"
