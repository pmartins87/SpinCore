...
"""Powered low-noise infoset confirmation on the pre-existing high-impact FAI block.

Structural inclusion is fixed from the earlier full-population reconciliation:
  * common Jammer FAI;
  * exactly one public action before FAI;
  * legal actions exactly FOLD/CALL (slots 0,1).

Within that frozen structure, classify only by Stage-B-minus-A production
fold-mass sign and uniformly sample B_MORE_FOLD and B_LESS_FOLD per seed.

Selection never uses sampled FAI action, terminal outcome, realized Q,
low-noise Q, or optimal class.

Reference:
  * JAMMER is hand-independent -> uniform compatible opponent hands;
  * 64 stratified hands x 8 future boards per anchor;
  * common canonical action-gap reference.
...
import audit_lt2_jammer_fai_fold_shift_infoset as base
...
def _group_summary(rows):
    ...
    out["fallback_a_rate"] = float(statistics.fmean(
        1.0 if r["stage_a"]["fallback_all_nonpositive"] else 0.0 for r in rows
    ))
    out["fallback_b_rate"] = float(statistics.fmean(
        1.0 if r["stage_b"]["fallback_all_nonpositive"] else 0.0 for r in rows
    ))
    return out
...
def main() -> int:
    ...
    for seed in FORENSIC_SEEDS:
        ...
        focused = {g: [] for g in groups}
        raw_group_counts = {"B_MORE_FOLD": 0, "B_LESS_FOLD": 0, "NO_FOLD_SHIFT": 0}

        for anchor0 in pool:
            group, fa, fb, delta = base._classify_shift(stage_a, stage_b, anchor0)
            raw_group_counts[group] += 1
        ...
        candidate_counts[str(seed)] = {
            "raw_common_fai": raw_group_counts,
            "focused": {g: len(focused[g]) for g in groups},
        }
        ...
        for gidx, group in enumerate(groups):
            ...
            if len(candidates) < need:
                raise RuntimeError(
                    f"seed {seed} focused group {group}: only {len(candidates)} "
                    f"candidates, need {need}"
                )
            chooser = random.Random(fd._mix64(seed, 0x5712C7A1, gidx))
    ...
    for i, anchor in enumerate(selected):
        print(
            f"FAI_STRUCTURAL_INFOSET anchor={i+1}/{len(selected)} "
        ...
        if legal != (0, 1):
            raise RuntimeError(f"structural legal drift: {legal}")
        if int(anchor["common_public_action_count"]) != 1:
            raise RuntimeError("structural path-length drift")

        fold_q = float(q[0].item())
        continue_q = float(q[1].item())
        value_delta = (
    ...
    summary = {
        "n": len(rows),
        "groups": {
            g: _group_summary([r for r in rows if r["fold_shift_group"] == g])
            for g in groups
        },
    }

    report = {
        "schema": "SPINCORE_LT2_JAMMER_FAI_STRUCTURAL_INFOSET_CONFIRMATION_V1",
        "stage_a": {
        ...
        "method": {
            ...
            "structural_focus": {
                ...
                "common_public_action_count": 1,
                "provenance": (
                    "pre-existing full-population reconciliation localization: "
                    "legal {FOLD,CALL} and path length 1 carried most resolved FAI loss"
                ),
            },
            "selection": (
                "within frozen structural focus, classify only by Stage-B-minus-A "
                "production fold-mass sign and uniformly sample each shift group; "
                "no sampled FAI action, terminal outcome, actual-deal Q, low-noise Q, "
                "or optimal class used for selection"
            ),
            "reference": {
            ...
            "primary_metric": "B_MORE_FOLD seed-cluster CI for policy_value_b_minus_a_chips",
            "directional_control": "B_LESS_FOLD policy_value_b_minus_a_chips",
        },
    ...
    print("=== JAMMER FAI STRUCTURAL INFOSET CONFIRMATION ===")
    print(f"anchors={len(rows)}")
    print(f"candidates_by_seed={candidate_counts}")
    for group in groups:
        ...
        print(
            ...
            f"fallback_A={s['fallback_a_rate']:.3f} "
            f"fallback_B={s['fallback_b_rate']:.3f}"
        )
        print(f"  optimal_classes={s['reference_optimal_class_counts']}")
    print("LT2_JAMMER_FAI_STRUCTURAL_INFOSET_CONFIRMATION_COMPLETE")
