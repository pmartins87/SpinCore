...
EpisodeScenario ScenarioSampler::sample(bool hu){EpisodeScenario s{};s.state.total_chips=cfg_.total_chips;s.state.small_blind=cfg_.small_blind;s.state.big_blind=cfg_.big_blind;s.state.blind_index=0;
 ...
 else {int x=1+(int)(next()%(std::uint64_t)(cfg_.total_chips-2));int rem=cfg_.total_chips-x;int y=1+(int)(next()%(std::uint64_t)(rem-1));int z=rem-y;s.state.stacks={x,y,z};s.state.game_is_hu=false;s.state.dead_players={-1,-1,-1};s.state.dead_player_count=0;s.dealer_id=(int)(next()%3);}validate_episode_scenario(s);return s;}
}
